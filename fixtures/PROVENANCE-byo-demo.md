...
- **Generated by**: the MONARK recorder on 2026-09-11 (grounding: ADR-M006 D10), **regenerated 2026-09-18
  for M012-f** (the harness now advertises a single-source `HARNESS_VERSION` `0.4.0`, so the `initialize`
  step's `serverInfo.version` moved off the misaligned `1.0.0`; grounding: ADR-M010 §2.3/§2.4). Recorder:
...
- **sha256 (LF-normalized)**: `daf8d3eabacbc601e608d01936d02c0f7ba78dfb5a0d6f5741ecea5fb4eef6d2`
  ...
  public tag `v0.4.0` / MCP registry `0.4.0`); a 5-char-for-5-char swap, so the length is unchanged and only

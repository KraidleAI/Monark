...
// F:\Clawpumptech\DESIGN-MODS-MONARK.md). The design's own profile→product mapping was ERRONEOUS and is
// thrown out; E-1 is the corrected, investor-ratified mapping. Each profile is a real client frame that
...
// register + panels are owned by Lot F-site-6 — here a profile only NAMES and links its product.
//
// `engineKeys` = which fleet agents power the product, used only to LIGHT the matching board cards. Every
// row is sourced (cited in docs/G1-lot-fsite-4.md):
//   firebreak, softlanding -> ukemi        DESIGN-MODS Mod #1 ("Softlanding/Firebreak -> Ukemi")
//   warden -> genkan                       DESIGN-MODS Mod #1 ("Warden -> Genkan")
//   ballast -> kyokusen                    DESIGN-MODS Mod #1 ("Ballast -> Kyokusen")
//   verdict -> mokugeki, kamae             DESIGN-MODS decision β (Verdict = Mokugeki × Kamae)
//   Attestation -> shogen                  design CORE L515 + memstack uid 28b02686 (VISAGE)
//   Hallmark   -> hikae                    design CORE L517 + memstack uid 28b02686 (VISAGE)
//   Threshold  -> hikae                    design CORE L516 + memstack uid 28b02686 (VISAGE)
//
...
export interface PickerProfile {
  ...
  /** The register key of the product (fingers); null for the VISAGE tier (Lot F-site-6 owns its register). */
...
export const PICKER_PROFILES: readonly PickerProfile[] = [
  ...
  // the investor rejected the meaning of (2026-09-09, uid 28b02686); F-site-6 renders "no engine named" for
  // these. Home lights only the backbone (gate + adapter) for a VISAGE profile — no unratified engine claim.
  // Escalated to the investor: whether Home should light an engine for the 3 VISAGE profiles, and which.
